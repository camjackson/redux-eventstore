...
export const get = (uri, logger) => (
  new Promise((resolve, reject) => {
    logger(`HTTP GET ${uri}`);
    const req = http.get({ ...url.parse(uri), headers: { Accept: atomJson } }, handleResult(resolve, reject, logger));

    req.on('error', reject);
  })
);

export const post = (uri, body, logger) => (
  new Promise((resolve, reject) => {
